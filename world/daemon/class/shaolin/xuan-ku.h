...
string ask_me()
{
	...
	say("\n玄苦又道：请你速去告知般若堂玄难大师，请他即刻招集般若堂诸罗汉于西练武场集合。\n");
        ...
        for(j=1; j<10; j++) 
	{
                if(!( room = find_object("/d/suburb/xkx/shaolin/luohan" + j)) )
                room = load_object("/d/suburb/xkx/shaolin/luohan" + j);
                
  		me->move(room);
                
		if( !objectp(monk = present(names[j-1], room) ))
		{
	        	me->move("/d/suburb/xkx/shaolin/luohan5");
        	        message_vision("\n玄苦大师走了过来。\n\n", fighter);
			return "真是对不起，罗汉堂中有人不在，无法举行罗汉大阵。\n";	
                ...
                monk->move("/d/suburb/xkx/shaolin/wuchang2");
                message("vision", monk->query("name") + "罗汉点了点头，快步走了出去。\n", room, monk);
	}
	
        me->move("/d/suburb/xkx/shaolin/wuchang");
...
int preparing(object me, object fighter)
{
	...
	if(!( room = find_object("/d/suburb/xkx/shaolin/wuchang2")) )
	room = load_object("/d/suburb/xkx/shaolin/wuchang2");
	monk = present("mu ren", room);
	monk->move("/d/suburb/xkx/shaolin/houdian");

	if(!( room = find_object("/d/suburb/xkx/shaolin/wuchang1")) )
	room = load_object("/d/suburb/xkx/shaolin/wuchang1");
	monk = present("mu ren", room);
	monk->move("/d/suburb/xkx/shaolin/houdian");

	if(!( room = find_object("/d/suburb/xkx/shaolin/wuchang")) )
	room = load_object("/d/suburb/xkx/shaolin/wuchang");
...
int fighting(object me, object fighter, int count)
{
	...
	if(!( room1 = find_object("/d/suburb/xkx/shaolin/wuchang2")) )
	room1 = load_object("/d/suburb/xkx/shaolin/wuchang2");

	if(!( room2 = find_object("/d/suburb/xkx/shaolin/wuchang")) )
	room2 = load_object("/d/suburb/xkx/shaolin/wuchang");
...
int do_back(object me)
{
	...
	message("vision", "\n玄苦大师挥了挥手， 罗汉堂长老们随即鱼贯离开练武场。\n", environment(me) );
        ...
        if(!( room1 = find_object("/d/suburb/xkx/shaolin/wuchang")) )
        room1 = load_object("/d/suburb/xkx/shaolin/wuchang");

        if(!( room2 = find_object("/d/suburb/xkx/shaolin/houdian")) )
        room2 = load_object("/d/suburb/xkx/shaolin/houdian");

        monk = present("daojue chanshi", room2);
	monk->move("/d/suburb/xkx/shaolin/wuchang");
        monk = present("mu ren", room2);
	monk->move("/d/suburb/xkx/shaolin/wuchang2");
        monk = present("mu ren", room2);
	monk->move("/d/suburb/xkx/shaolin/wuchang1");
	...
	for(i=1; i<10; i++) 
	{
                if(!( room2 = find_object("/d/suburb/xkx/shaolin/luohan" + i)) )
                room2 = load_object("/d/suburb/xkx/shaolin/luohan" + i);
